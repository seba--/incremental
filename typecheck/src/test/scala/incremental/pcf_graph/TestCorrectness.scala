...
class TestCorrectness(classdesc: String, checkerFactory: TypeCheckerFactory[Type]) extends FunSuite with BeforeAndAfterEach {
  ...
  typecheckTest("\\x. 10+5", Abs('x, Add(Num(10), Num(5))))(TFun(TUnifiable('x$0), TNum))
  typecheckTest("\\x. x+x", Abs('x, Add(Var('x), Var('x))))(TFun(TNum, TNum))
  typecheckTestError("\\x. err+x", Abs('x, Add(Var('err), Var('x))))
  typecheckTest("\\x. \\y. x y", Abs('x, Abs('y, App(Var('x), Var('y)))))(TFun(TFun(TUnifiable('x$1), TUnifiable('x$2)), TFun(TUnifiable('x$1), TUnifiable('x$2))))
  ...
  typecheckTest("\\y. y", Abs('y, Var('y)))(TFun(TUnifiable('x$0), TUnifiable('x$0)))
