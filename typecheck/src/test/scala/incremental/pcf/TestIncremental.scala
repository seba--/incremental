...
class TestIncremental[CS <: ConstraintSystem[CS]](classdesc: String, checkerFactory: TypeCheckerFactory[CS]) extends FunSuite with BeforeAndAfterEach {
  ...
  def incTypecheckTest(desc: String, e: =>Node_[_])(expected: equality.Type)(consCount: Int): Unit = incTypecheckTest(desc, e, Unit)(expected)(consCount)
  def incTypecheckTest(desc: String, e: =>Node_[_], mod: =>Unit)(expected: equality.Type)(consCount: Int): Unit =
  ...
  incTypecheckTest("\\x. add4", fun1)(TFun(UVar(CVar('x$1)), TNum))(0)
  ...
  incTypecheckTest("(\\x. x+x+x) 3 + (1 + 2)", app1, fun1.kids(0) = Add(add4, Var('x)))(TNum)(5)
  ...
  incTypecheckTest("factorial base 1", fac, fac match {case Fix(Abs(_, _, Abs(_, _, e@If0(_,_,_)))) => e.kids(1) = Num(1)})(TFun(TNum, TNum))(4)
}
