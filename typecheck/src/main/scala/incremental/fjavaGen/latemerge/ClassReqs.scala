...
import incremental.fjavaGen.CName
import incremental.fjavaGen.EqualCName
...
case class ClassReqs (
  ...
  def satisfyField(field: FieldCReq): (ClassReqs, Seq[Constraint]) = satisfyFields[FieldCReq](field, fields, x=>copy(fields=x))
  ...
  private def satisfyFields[T <: CReq[T]](creq1: T, crs: Set[T], make: Set[T] => ClassReqs): (ClassReqs, Seq[Constraint]) = {
    var cons = Seq[Constraint]()
    val newcrs = crs flatMap ( creq2 =>
      if (creq1.canMerge(creq2)) {
        if (creq1.asInstanceOf[FieldCReq].typ.isGround){
          creq1.assert(creq2) foreach (c => cons = cons :+ c)
          creq2.alsoNot(creq1.cls)
        }
        else {
          cons = cons :+ Conditional(creq2.cls, creq2.cond, EqualCName(creq1.asInstanceOf[FieldCReq], creq2.asInstanceOf[FieldCReq]))
          creq2.alsoNot(creq1.cls)
        }
      }
      else
