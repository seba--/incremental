  ...
  def typecheckStep(e: Node_[Result]): StepResult = e.kind match{
   ...
   case MethodDec =>
     ...
     creqs.get(C) match {
       ...
       case None =>
       case Some
         cons = EqConstraint(retD, retT) +: cons
     ...
     //if (e.lits(3).isInstanceOf[Seq[Symbol]]){
       //val params = e.lits(3).asInstanceOf[Seq[Symbol]]
       //for (i <- 0 until params.size ){
        // val x = params(i)
         //val xC = freshCName()
         //param += (CVar[Param](x) -> xC)
...
//        reqs.get(x) match {
//          case None =>package incremental.FJava
...
//      }
      ...
      val cld : ClassDecl = (null, Map(), Map(m -> (retT, param)))

     val (mcons, mcreqs) = mergeCReqMaps(creqs, Map(C -> cld) )

     (C, restReqs, mcreqs, mcons ++ cons :+ EqConstraint)
