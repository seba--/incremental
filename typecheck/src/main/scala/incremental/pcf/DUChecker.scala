...
abstract class DUChecker[CS <: ConstraintSystem[CS]] extends TypeChecker[CS] {
  ...
  type TError = String
  type Result = (Type, CS)
  override type Res = PCFCheck.Result
  ...
  def typecheckRec(e: Node_[Constraint, CS, Res], ctx: TCtx): Result = {
  ...
  def typecheckStep(e: Node_[Constraint, CS, Res], ctx: TCtx): StepResult = e.kind.asInstanceOf[NodeKind[Constraint, PCFCheck.Result]] match {
