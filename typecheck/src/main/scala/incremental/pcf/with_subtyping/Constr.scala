...
import incremental.pcf.with_subtyping.UVar
import incremental.{ConstraintSystem, Util}
import TypeOps._
 ...
 */
abstract class SubtypeConstraintSystem extends ConstraintSystem[Type] {
...
object CS extends SubtypeConstraintSystem {
  def mkInstance = new Instance {
    def _mergeReqMaps(reqs1: CS.Requirements, reqs2: CS.Requirements) = {
      var mcons = emptyCSet
      var mreqs = reqs1
      for ((x, r2) <- reqs2)
        reqs1.get(x) match {
    ...
    def emptyCSet = new CSet(stats, gen)

    val gen: CS.Gen = new GenPlusMinus
    val stats: CS.Statistics = new Statistics
  }

  class CSet(val stat: Statistics, val gen: Gen) extends CSetAlg[CSet] {
    import gen._
    import stat._
