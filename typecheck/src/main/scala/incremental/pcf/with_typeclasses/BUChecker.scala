...
abstract class BUChecker[CS <: ConstraintSystem[CS]] extends TypeChecker[CS] {
  ...
  def typecheckRec(e: Node_[Result]): Unit = {
    ...
    var resreq = Map[Symbol, Seq[Type]]()
    reqs foreach (r => resreq += (r._1 -> r._2.map(_.subst(cs.substitution))))
    e.typ = (cs applyPartialSolution t, resreq, cs.propagate)
  }
  ...
  def typecheckStep(e: Node_[Result]): StepResult = e.kind match {
    ...
    case Abs if (e.lits(0).isInstanceOf[Symbol]) =>
      ...
      reqs.get(x) match {
        ...
        case Some(treq) =>
          ...
          if (e.lits.size == 2) {
            (TFun(treq.head, t), otherReqs, Seq(EqConstraint(e.lits(1).asInstanceOf[Type], treq.head)))
          }
          else
            (TFun(treq.head, t), otherReqs, Seq())
      }
    ...
    case Let =>
      ...
      mreq.get(u) match {
        ...
        case Some(treq) =>
          otherReqs = mreq - u
          rescons ++= Seq(EqConstraint(t1, treq.head))
      }
      (t2, otherReqs, rescons)
    ...
    case Inst =>
      ...
      val (cons, mreq) = mergeReqMaps(reqs1, reqs2, Map(o -> Seq(t1)))
      (t2, mreq, cons)
  }
