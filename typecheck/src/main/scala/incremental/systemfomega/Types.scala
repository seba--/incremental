...
case class TTAbs(alpha: Symbol, t: Type) extends Type {
  def freeTVars = t.freeTVars - alpha

  def occurs(x2: Symbol) = alpha == x2 || t.occurs(x2)

  def subst(s: Map[Symbol, Type]) = TTAbs(alpha, t.subst(s - alpha))

  def unify(other: Type, s: TSubst) = other match {
    case TTAbs(alpha2, t2) => t.unify(t2, s + (alpha2 -> TVar(alpha)))
    case UVar(_) => other.unify(this, s)
    case _ => never(EqConstraint(this, other))
  }
}

case class TTApp(tfun: Type, targ: Type) extends Type {
  def freeTVars = tfun.freeTVars ++ targ.freeTVars

  def occurs(x2: Symbol) = tfun.occurs(x2) || targ.occurs(x2)

  def subst(s: Map[Symbol, Type]) = TTApp(tfun.subst(s), targ.subst(s))

  def unify(other: Type, s: TSubst) = other match {
    case TTApp(tfun2, targ2) =>
      val sol1 = tfun.unify(tfun2, s)
      val sol2 = targ.unify(targ2, s)
      sol1 ++ sol2
    case UVar(_) => other.unify(this, s)
    case _ => never(EqConstraint(this, other))
  }
