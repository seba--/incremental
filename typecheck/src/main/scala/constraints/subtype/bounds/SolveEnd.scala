...
case class SolveEndCS(substitution: TSubst, bounds: Map[Symbol, (LBound, UBound)], never: Seq[Constraint]) extends ConstraintSystem[SolveEndCS] {
  ...
  def mergeSubsystem(that: SolveEndCS) = {
    var current = SolveEndCS(substitution ++ that.substitution, bounds, never ++ that.never)
    ...
    for((tv, (thatL, thatU)) <- that.bounds) {
      if (bounds.isDefinedAt(tv)) {
        ...
        current = SolveEndCS(current.substitution, current.bounds + (tv -> (thatL, thatU)), current.never)
    }
  ...
  def tryFinalize = {
    val (res, time) = Util.timed {
      ...
      SolveEndCS(substitution, finalbounds, never).saturateSolution
    }
  ...
  private def saturateSolution = {
    ...
    while (sol.nonEmpty) {
      ...
      current = SolveEndCS(current.substitution ++ sol, temp.bounds, current.never ++ newnever ++ temp.never)
  ...
  def addLowerBound(v: Symbol, t: Type) = {
    ...
    val cs = SolveEndCS(substitution, newbounds, newnever)
